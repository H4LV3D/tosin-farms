...
export default function AddressPage() {
  ...
  const setDefaultMutation = useMutation({
    mutationFn: (address: SavedAddress) =>
      updateUserAddress(address.id, { ...address, isDefault: true }),
      ...
      <PageHeader
        ...
        action={
          <Button
            ...
            className="bg-amber-700 hover:bg-amber-600 text-white gap-2 h-12 px-8! rounded-md font-medium"
          >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {addresses.map((address, index) => (
            <Card
              key={address.id}
              className={cn(
                "border-2 p-4 transition-all duration-300 overflow-hidden relative group rounded-3xl",
                address.isDefault
