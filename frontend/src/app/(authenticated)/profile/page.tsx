...
import { Loader2, User, Mail, Camera, Save, Pencil, Fingerprint } from "lucide-react";
import { cn } from "@/lib/utils";
import { PasskeySetup } from "@/components/shared/PasskeySetup";
import { AuthenticatedPageContainer, PageHeader } from "@/components/shared/AuthenticatedPageLayout";
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <Card className="border-none shadow-2xl shadow-stone-200/50 rounded-3xl overflow-hidden bg-white">
                        <div className="h-24 bg-gradient-to-r from-amber-600 to-amber-800" />
                        <CardContent className="pt-0 -mt-12 flex flex-col items-center text-center px-6 pb-8">
                            ...
                            <h2 className="text-xl font-bold text-[#1c1917] mt-4">{user.name || "Tosi Farmer"}</h2>
                            <p className="text-[10px] text-amber-700 font-black uppercase tracking-[0.2em] mt-2 bg-amber-50 px-3 py-1 rounded-full">{user.role}</p>
                ...
                <div className="lg:col-span-2 space-y-8">
                    <Card className="border-none shadow-sm rounded-3xl bg-white">
                        <CardHeader className="px-8 pt-8 pb-4">
                            <CardTitle className="text-xl font-bold text-[#1c1917]">Personal Information</CardTitle>
                        </CardHeader>
                        <CardContent className="px-8 pb-8">
                            <form onSubmit={handleSubmit} className="space-y-6">
                                <div className="space-y-2">
                                    <Label htmlFor="name" className="text-[10px] font-black uppercase tracking-widest text-stone-500">Full Name</Label>
                                    <Input
                                        id="name"
                                        value={formData.name}
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="avatarUrl" className="text-[10px] font-black uppercase tracking-widest text-stone-500">Profile Picture URL</Label>
                                    <Input
                                        id="avatarUrl"
                                    ...
                                    />
                                    {isEditing && (
                                        <p className="text-[11px] text-stone-400 italic font-medium">Use a direct image URL (e.g., Unsplash) for best result.</p>
                                    )}
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="bio" className="text-[10px] font-black uppercase tracking-widest text-stone-500">Bio</Label>
                                    <textarea
                                        id="bio"
                                        value={formData.bio}
                    ...
                    <Card className="border-none shadow-sm rounded-3xl bg-white">
                        <CardHeader className="px-8 pt-8 pb-4">
                            <CardTitle className="text-xl font-bold text-[#1c1917]">Security & Passkeys</CardTitle>
                        </CardHeader>
                        <CardContent className="px-8 pb-8 space-y-8">
                            <div className="flex items-start gap-5 p-6 rounded-[2rem] bg-stone-50/50 border border-stone-100 transition-colors hover:bg-stone-50">
                                ...
                                <div className="space-y-1">
                                    <h4 className="text-sm font-black text-[#1c1917] uppercase tracking-tight">Passkey Authentication</h4>
                                    <p className="text-xs text-stone-500 leading-relaxed font-medium italic">
                                        Passkeys are a safer replacement for passwords. Use biometrics like fingerprint or face recognition to sign in.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <PasskeySetup
                                    onSuccess={() => {
                                        setStatus({ type: "success", message: "Passkey added successfully!" });
                                    }}
                                    variant="outline"
                                    className="w-full"
                                />
