...
export function OrderReviewStep({
    ...
    const shipping = SHIPPING_COSTS[shippingData.dispatchType] ?? 2500;
            ...
            <div className="bg-stone-50 rounded-2xl border border-stone-100 p-5 mb-5">
                ...
                <div className="space-y-2.5">
                    <div className="flex items-start gap-2.5 text-sm">
                        ...
                        <div>
                            <p className="font-semibold text-[#1c1917]">{shippingData.fullName}</p>
                    ...
                    <div className="flex items-center gap-2.5 text-sm">
                        ...
                        <span className="text-stone-600">
                            {shippingData.dispatchType === "DHL" ? "DHL Express" : "FedEx"} — 3–5 business days
                        </span>
                    </div>
                    {shippingData.note && (
            ...
            <div className="bg-amber-50/60 border border-amber-100 rounded-2xl p-5 mb-6">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-[#1c1917]">₦{subtotal.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span className="text-stone-500">Shipping ({shippingData.dispatchType})</span>
                        <span className="font-medium text-[#1c1917]">₦{shipping.toLocaleString()}</span>
                    </div>
                    <div className="border-t border-amber-100 pt-2 mt-2 flex justify-between">
                        <span className="font-bold text-[#1c1917]">Total</span>
                        <span className="font-bold text-xl text-amber-700">₦{total.toLocaleString()}</span>
                    </div>
                </div>
            </div>
