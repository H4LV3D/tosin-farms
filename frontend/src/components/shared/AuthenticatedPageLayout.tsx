    ...
    <div className="flex flex-col gap-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-lato font-bold text-[#1c1917] tracking-tight">
            {title}
          </h1>
          {description && (
            <p className="text-stone-500 font-normal text-lg max-w-2xl leading-relaxed">
              {description}
            </p>
          )}
        </div>
      ...
      <CardContent className="flex flex-col items-center max-w-md mx-auto space-y-8">
        ...
        {actionHref ? (
          <Link href={actionHref}>
            <Button className="h-14 px-10! rounded-md bg-amber-700 hover:bg-amber-600 text-white font-black uppercase tracking-wider text-sm gap-3 shadow-amber-900/30 transition-all hover:scale-105 active:scale-95 group">
              {actionLabel}
          ...
          <Button
            onClick={onAction}
            className="h-14 px-10 rounded-md bg-amber-700 hover:bg-amber-600 text-white font-black uppercase tracking-wider text-sm gap-3 shadow-amber-900/30 transition-all hover:scale-105 active:scale-95 group"
          >
            {actionLabel}
            <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
          </Button>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {Array.from({ length: count }).map((_, i) => (
          <div
            key={i}
            className="aspect-4/5 bg-white rounded-[3rem] p-2 shadow-sm border border-stone-100 flex flex-col gap-4"
          >
